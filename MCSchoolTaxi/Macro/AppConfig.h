//
//  AppConfig.h
//  StarGroups
//
//  Created by fenguo on 15-1-19.
//  Copyright (c) 2015年 fenguo. All rights reserved.
//

#ifndef StarGroups_AppConfig_h
#define StarGroups_AppConfig_h

#import <Foundation/Foundation.h>
#import "NSArray+Log.h"
#import "NSDictionary+Log.h"
#import "AppDelegate.h"
#import "MCNavViewController.h"
#import "BaseViewController.h"
#import "MJRefresh.h"
#import "MJExtension.h"
#import "UIImageView+WebCache.h"
#import "UIButton+WebCache.h"
#import "CommonUtil.h"
#import "MCIucencyView.h"
#import "MCToolsManage.h"
#import "SDImageCache.h"
#import "UIImage+GIF.h"
#import "UIImageView+WebCache.h"
#import "UIImage+MultiFormat.h"
#import "UIImageView+AFNetworking.h"
#import "UIButton+WebCache.h"
#import "MCNetworkManager.h"
#import "MCDataMmodel.h"
#import "UserModel.h"
#import "MCMApManager.h"
#import "HClActionSheet.h"
#import "AlertViewExtension.h"

#import <AMapFoundationKit/AMapFoundationKit.h>
#import <MAMapKit/MAMapKit.h>
#import <AMapSearchKit/AMapSearchKit.h>



#define AppMapKey  @"b8d658c580aed60b3957aa6f278a05f0"


#define AppTextCOLOR      RGBCOLOR(127, 125, 147);

#define AppRegTextCOLOR      RGBCOLOR(207, 0, 51);

#define AppFont     [UIFont systemFontOfSize:14]
#define AppBgCOLOR      RGBCOLOR(250, 247, 247);

#define ApphomeBgCOLOR     RGBCOLOR(240, 243, 244);




#define AppMCNACOLOR    kUIColorFromRGB(0xf6f6f6);// 导航栏背景

#define AppMCBgCOLOR    kUIColorFromRGB(0xf8f8f8);//界面背景
#define AppCOLOR      kUIColorFromRGB(0x40aefc);//主蓝色


#define AppMCNATitleCOLOR    kUIColorFromRGB(0x703c03);// 导航栏title



#define AppVersions @"v1.0.0";



#define AppURL @"http://119.29.0.81:7098/zhongyi/api/app/"



#endif
